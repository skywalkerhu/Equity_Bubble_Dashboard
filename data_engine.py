# # This script is executed every 12 hours by GitHub Actions.
...
def fetch_global_valuations():
    ...
    try:
        treasury_df = web.DataReader('DGS3', 'fred', pd.Timestamp.now() - pd.DateOffset(days=10))
        treasury_clean = treasury_df.dropna()
        treasury_3y = float(treasury_clean.iloc[-1, 0])
        treasury_asof = treasury_clean.index[-1].date().isoformat()
        valuations['US3Y'] = {'name': '3-Yr US Treasury', 'pe': None, 'yield': treasury_3y}
    except Exception as e:
        print(f"⚠️ Treasury fetch failed, keeping last known value: {e}")
        valuations.setdefault('US3Y', {'name': '3-Yr US Treasury', 'pe': None, 'yield': None})
    ...
    # Freshness metadata lives in its own file so valuations.json keeps its
    # exact existing shape (ticker -> {name, pe, yield}) and nothing that
    # iterates over it needs to change.
    meta = {
        'generated_at': datetime.now(timezone.utc).isoformat(timespec='seconds'),
        'treasury_asof': treasury_asof,
        'pe_live': pe_live,
        'pe_total': len(indices),
    }
    with open('data/meta.json', 'w') as f:
        json.dump(meta, f, indent=2)
    print(f"✅ Valuations written: {pe_live}/{len(indices)} P/Es live, treasury as of {treasury_asof}")
